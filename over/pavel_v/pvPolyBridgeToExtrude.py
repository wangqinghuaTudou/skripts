            ...
            if history:
                ...
                if curve:
                    ...
                    if bridge:
                        ...
                        if skin_cluster:
                            #curve_info = cmds.listConnections( curve + ".worldSpace[0]", type="curveInfo", source=False, destination=True )
                            bridge_history = cmds.listHistory( bridge )
                            curve_info = cmds.ls(bridge_history, type = 'curveInfo')
                            if curve_info:
                                range_node = cmds.listConnections( curve_info[0] + ".arcLength", source=False, destination=True )
                                if range_node:
                                    range_node = range_node[0]
                                    taper_offset_node = cmds.listConnections( bridge + ".taper", source=True, destination=False )
                                        ...
                                        if duplicate_node_shape:
                                            ...
                                            if cmds.objExists( duplicate_node_orig_shape ):
                                                ...
                                                if face_list:
                                                    ...
                                                    om.MGlobal.displayWarning( 'No "faces" on duplicateShape node.' )
                                                    failed = True
                                            else:
                                                om.MGlobal.displayWarning( 'No "duplicateOrigShape" node.' )
                                                failed = True
                                        else:
                                            om.MGlobal.displayWarning( 'No "duplicateShape" node.' )
                                            failed = True
                                    else:
                                        om.MGlobal.displayWarning( 'No "taperOffset" node.' )
                                        failed = True
                                else:
                                    om.MGlobal.displayWarning( 'No "range" node.' )
                                    failed = True
                            else:
                                om.MGlobal.displayWarning( 'No "curveInfo" node.' )
                                failed = True
                        else:
                            om.MGlobal.displayWarning( 'No "bridge" node.' )
                            failed = True
                        if failed:
                            #cmds.delete( duplicate_node )
                            om.MGlobal.displayWarning ('Failed')
                    else:
                        om.MGlobal.displayWarning ('Warning: %s has no contain any bridge node in history.'%shape)
                else:
                    om.MGlobal.displayWarning ('Warning: %s has no contain any nurbsCurve in history.'%shape)
            else:
                om.MGlobal.displayWarning ('Warning: %s has no history.'%shape)
        else:
            om.MGlobal.displayWarning ('Warning: %s has no any child.'%node)
    return result
...
def TransferSkinClusterInfluence( source, target ):
    ...
    if cmds.ls( source, type="transform" ) and cmds.ls( target, type="transform" ):
        connections = cmds.listConnections( source + ".worldMatrix[0]", source=False, destination=True )
        skin_cluster_list = []
        if connections:
