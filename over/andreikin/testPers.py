...
def  keyCtTest(ctList):   #проверка контроллеров на ключи
    keyCt=[]
    for ct in ctList:
...
def  zeroCtTest(ctList):   #проверка контроллеров на ненулевые значения 
    zeroVals = ['tx', 'ty', 'tz', 'rx', 'ry', 'rz' ]
...
def uniqCtTest():     # проверка контроллеров на уникальность   
...
def geoSmoothnessOf (): #убираем сглаженность
    for mesh in cmds.ls (type=['mesh',]):
        cmds.setAttr(mesh+'.dsm', 0)

def setIkToleranse (): #проверка ик солвера на соотсветствие значения
    cmds.setAttr ("ikSCsolver.tolerance", 0.0000000001)    

def jntVisibility(): #проверка костей на видимость
    jntList = []
