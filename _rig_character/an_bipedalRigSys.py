...
def an_bipedalRigSys():
    ...
    cmds.window (win, t="Bipedal Rig System v1.00", width=420,  height=420, s=False, rtf=True, menuBar=True )
    cmds.columnLayout ('columnLayoutName'  , adjustableColumn=True)
    cmds.canvas( height=5 )
    cmds.floatSliderGrp('ISG_gScale', label='Global scale:', field=True, min=0.1, max=10,  v=1 , cw = [(1, 124), (2, 50) ], enable= True )
    cmds.canvas( height=5 )
...
def an_SkeletonTemplate(aktion):
    ...
    obj = An_Skeleton()
    if aktion == 'template':
        obj.templateCtrl(gScale=gScale)
    if aktion == 'skelet':
    ...
    an_bipedalRigSys()
    cmds.floatSliderGrp('ISG_gScale', e=True, v= gScale )
